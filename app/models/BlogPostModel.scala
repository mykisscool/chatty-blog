...
/** Case class for blog post
  ...
  */
case class Post(fullname: String,
                nickname: String,
                title: String,
                slug: String,
                image: String,
...
class BlogPostModel @Inject()(dbapi: DBApi, configuration: Configuration) {
  ...
  private def parser(isPost: Boolean = false): RowParser[Post] = {
    ...
    case fullname ~ nickname ~ title ~ slug ~ image ~ content ~ created_at =>
      Post(fullname, nickname, title, slug, image, if (isPost) content else truncateContent(content), prettyDate(created_at))
    }
  }
